"""Treasurer flows: record donations/expenses, browse & correct recent entries.
...
# --------------------------------------------------------------------------
# Record a donation: amount -> optional note -> confirm
# --------------------------------------------------------------------------


@router.message(F.text == BTN_ADD_DONATION)
async def start_add_donation(message: Message, state: FSMContext) -> None:
    await state.set_state(DonationEntryStates.awaiting_amount)
    await message.answer(
        "Ehson summasini kiriting (so'm). Masalan: 50000", reply_markup=cancel_only()
    )


@router.message(DonationEntryStates.awaiting_amount, F.text == BTN_CANCEL)
@router.message(DonationEntryStates.awaiting_note, F.text == BTN_CANCEL)
@router.message(DonationEntryStates.awaiting_confirm, F.text == BTN_CANCEL)
async def cancel_add_donation(message: Message, state: FSMContext, session: AsyncSession) -> None:
    await state.clear()
    await message.answer("Bekor qilindi.")
    await show_main_menu(message, session)


@router.message(DonationEntryStates.awaiting_amount)
async def donation_amount_entered(message: Message, state: FSMContext) -> None:
    amount = _parse_amount(message.text or "")
    if amount is None:
        await message.answer("Summa noto'g'ri. Masalan: 50000")
        return
    await state.update_data(amount=str(amount))
    await state.set_state(DonationEntryStates.awaiting_note)
    await message.answer("Izoh qo'shasizmi? (ixtiyoriy)", reply_markup=skip_or_cancel())


async def _ask_donation_confirm(message: Message, state: FSMContext) -> None:
    data = await state.get_data()
    await state.set_state(DonationEntryStates.awaiting_confirm)
    note_line = f"\nIzoh: {data['note']}" if data.get("note") else ""
    await message.answer(
        f"Tasdiqlaysizmi?\nSumma: {Decimal(data['amount']):,.0f} so'm{note_line}",
        reply_markup=confirm_cancel(),
    )


@router.message(DonationEntryStates.awaiting_note, F.text == BTN_SKIP)
async def donation_note_skipped(message: Message, state: FSMContext) -> None:
    await state.update_data(note=None)
    await _ask_donation_confirm(message, state)


@router.message(DonationEntryStates.awaiting_note)
async def donation_note_entered(message: Message, state: FSMContext) -> None:
    await state.update_data(note=(message.text or "").strip() or None)
    await _ask_donation_confirm(message, state)


@router.message(DonationEntryStates.awaiting_confirm, F.text == BTN_CONFIRM)
async def donation_confirmed(message: Message, state: FSMContext, session: AsyncSession) -> None:
    if message.from_user is None:
        return
    data = await state.get_data()
    use_case = RecordDonationUseCase(SqlAlchemyDonationRepository(session))
    try:
    ...
    await state.clear()
    await message.answer(f"✅ Qabul qilindi: {donation.amount} so'm\nKod: D{donation.id}")
    await show_main_menu(message, session)
...
async def _ask_expense_confirm(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        ...
        f"Izoh: {data['description']}{receipt_line}",
        reply_markup=confirm_cancel(),
    )
...
# --------------------------------------------------------------------------
# Recent entries: browse + correct (remove) a mistaken entry
# --------------------------------------------------------------------------
...
async def ask_confirm_removal(message: Message, state: FSMContext, session: AsyncSession) -> None:
    ...
    if kind == "D":
        ...
        if donation is None:
            ...
            return
        note_suffix = f" ({donation.note})" if donation.note else ""
        ...
        if expense is None:
            await message.answer(f"{code} topilmadi.")
            return
        summary = f"{expense.amount} so'm ({expense.description})"
